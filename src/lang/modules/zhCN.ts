export default {
    system: {
        ["login"]: "登录",
        ["not-power"]: "没有权限",
        ["not-found"]: "未找到页面",
        ["zh-CN"]: "中文",
        ["en-US"]: "English",
        ["refresh"]: "刷新",
        ["reload"]: "重新加载",
        ["close-current"]: "关闭当前",
        ["close-current-tab"]: "关闭当前标签页",
        ["close-left-side"]: "关闭左侧",
        ["close-left-tab"]: "关闭左侧标签页",
        ["close-right-side"]: "关闭右侧",
        ["close-right-tab"]: "关闭右侧标签页",
        ["close-other"]: "关闭其它",
        ["close-other-tab"]: "关闭其他标签页",
        ["close-all"]: "关闭全部",
        ["close-all-tab"]: "关闭全部标签页",
        ["content-fullscreen"]: "内容区全屏",
        ["exit-content-fullscreen"]: "退出内容区全屏",
        ["switch-to-night-mode"]: "切换黑夜模式",
        ["switch-to-daytime-mode"]: "切换白天模式",
        ["full-screen"]: "全屏",
        ["exit-full-screen"]: "退出全屏",
        ["system-settings"]: "系统设置",
        ["theme-settings"]: "主题设置",
        ["navigate mode"]: "导航模式",
        ["personal-information"]: "个人信息",
        ["change-password"]: "修改密码",
        ["project-address"]: "项目地址",
        ["logout"]: "退出登录",
        ["notice"]: "通知",
        ["message"]: "消息",
        ["backlog"]: "待办",
        ["switch-language-to-preview"]: "请切换语言来预览国际化效果",
        ["please-enter-something"]: "请输入",
        ["color weakness mode"]: "色弱模式",
        ["grey mode"]: "灰色模式",
        ["sidebar is dark"]: "侧边栏深色",
        ["page transition"]: "页面过渡",
        ["interface-settings"]: "界面设置",
        ["menu-folding"]: "菜单折叠",
        ["menu-accordion"]: "菜单手风琴",
        ["breadcrumb"]: "面包屑",
        ["tab-bar"]: "标签栏",
        ["page-footer"]: "页脚",
        ["menu-layout"]: "菜单布局",
        ["menu-layout-side"]: "左侧菜单",
        ["menu-layout-top"]: "顶部菜单",
        ["menu-layout-mix"]: "混合菜单",
        ["theme-color"]: "主题色",
        ["color-weakness-mode"]: "色弱模式",
        ["grey-mode"]: "灰色模式",
        ["sidebar-dark"]: "侧边栏深色",
        ["page-transition"]: "页面过渡",
        ["transition-light"]: "轻过渡",
        ["transition-card"]: "卡片",
        ["transition-fade"]: "渐退",
        ["watermark-settings"]: "水印设置",
        ["watermark-color"]: "水印颜色",
        ["watermark-text"]: "水印文案",
        ["watermark-size"]: "水印大小",
        ["watermark-angle"]: "水印角度",
        ["watermark-gap"]: "水印间隙",
        ["system settings"]: "系统设置",
        ["anti-debugging"]: "防调试",
        ["please select"]: "请选择",
        ["global-tenant"]: "全局租户",
        ["switch-tenant"]: "切换租户",
        ["switch-tenant-title"]: "切换租户",
        ["switch-tenant-confirm"]: "确定要切换到租户 \"{name}\" 吗？",
        ["switch-global-tenant-confirm"]: "确定要切换到全局租户吗？",
        ["cancel"]: "取消",
        ["confirm"]: "确定"
    },
    menu: {
        ["home"]: "首页",
        ["file"]: "文件管理",
        ["document-library"]: "文件库",
        ["table"]: "列表管理",
        ["common-table"]: "普通列表",
        ["custom-table"]: "自定义列表",
        ["form"]: "表单管理",
        ["common-form"]: "普通表单",
        ["step-form"]: "分步表单",
        ["multilevel"]: "多级菜单",
        ["second-1"]: "二级菜单-1",
        ["second-2"]: "二级菜单-2",
        ["third-1"]: "三级菜单-1",
        ["third-2"]: "三级菜单-2",
        ["third-3"]: "三级菜单-3",
        ["third-4"]: "三级菜单-4",
        ["third-5"]: "三级菜单-5",
        ["third-6"]: "三级菜单-6",
        ["third-7"]: "三级菜单-7",
        ["third-8"]: "三级菜单-8",
        ["third-9"]: "三级菜单-9",
        ["component"]: "组件管理",
        ["player"]: "播放器",
        ["print"]: "打印",
        ["draggable"]: "拖拽",
        ["editor"]: "富文本",
        ["newbie"]: "新手指引",
        ["icon-selector"]: "图标选择器",
        ["user-center"]: "用户中心",
        ["fingerprintjs2"]: "浏览器指纹",
        ["barcode"]: "条形码",
        ["qrcode"]: "二维码",
        ["pinyin"]: "拼音转换",
        ["markdown"]: "Markdown",
        ["directive"]: "自定义指令",
        ["anti-shake"]: "防抖",
        ["throttle"]: "节流",
        ["test-directive"]: "测试指令",
        ["personal"]: "个人中心",
        ["userinfo"]: "用户信息",
        ["user-settings"]: "用户设置",
        ["system"]: "系统管理",
        ["account"]: "账号管理",
        ["role"]: "角色管理",
        ["menu"]: "菜单管理",
        ["division"]: "部门管理",
        ["dictionary"]: "字典管理",
        ["log"]: "系统日志",
        ["about"]: "关于项目",
        ["link"]: "外部链接",
        ["internal"]: "内部",
        ["uigradients"]: "渐变配色",
        ["color-taking-tool"]: "取色工具",
        ["grid-generator"]: "网格生成器",
        ["external"]: "外部",
        ["vue"]: "vue官网",
        ["vite"]: "vite官网",
        ["github"]: "github",
        ["SnowAdmin-Docs"]: "SnowAdmin文档",
        ["juejin"]: "掘金",
        ["disable-menu"]: "停用菜单",
        ["hide-menu"]: "隐藏菜单",
        ["system-monitor"]: "系统监控",
        ["onlineuser"]: "在线用户",
        ["crontab"]: "定时任务",
        ["crontab-logs"]: "任务日志",
        ["sysjobs"]: "任务调度",
        ["jobslist"]: "任务管理",
        ["joblog"]: "任务日志",
        ["i18n"]: "国际化",
        ["amap"]: "高德地图",
        ["permission"]: "权限测试",
        ["btn-perm"]: "按钮权限",
        ["admin-page"]: "管理员页面",
        ["common-page"]: "普通角色页面",
        ["functions"]: "常用功能",
        ["routing-operation"]: "路由操作",
        ["common-route"]: "普通路由",
        ["dynamic-route"]: "动态路由",
        ["audio"]: "音频",
        ["recorder"]: "录音",
        ["virtual-list"]: "虚拟列表",
        ["common-tools"]: "常用工具类",
        ["tree-tools"]: "树处理工具类",
        ["file-tools"]: "文件工具类",
        ["verify-tools"]: "校验工具类",
        ["common-layouts"]: "常用布局",
        ["data-screen"]: "数据大屏",
        ["thin-preview"]: "极简版预览",
        ["test"]: "测试",
        ["system-config"]: "系统配置",
        ["file-manager"]: "文件管理",
        ["api-management"]: "接口管理",
        ["plugin-example"]: "插件示例",
        ["tenant"]: "租户管理",
        ["codegen"]: "代码生成",
        ["plugins-manager"]: "插件管理",
        ["QQQQ"]: "哈哈哈"
    },
};
